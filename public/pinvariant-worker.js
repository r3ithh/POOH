...
let pinvariantsCore = null;
try {
  if (typeof importScripts === "function") {
    importScripts(
      "../src/core/i18n.js",
      "../src/core/worker-i18n.js",
      "../src/core/pinvariants.js"
    );
  }
  if (typeof self !== "undefined" && self.PoohPinvariantsCore) {
    pinvariantsCore = self.PoohPinvariantsCore;
  }
} catch (error) {
  pinvariantsCore = null;
}
...
`;
  ...
  static async create() {
    if (!self.navigator || !self.navigator.gpu) {
      throw new Error(workerT("core.pinvariant.webgpuUnavailable"));
    }
    const adapter = await self.navigator.gpu.requestAdapter();
    if (!adapter) {
      throw new Error(workerT("core.pinvariant.webgpuAdapterMissing"));
    }
  ...
  async computeDots(candidates, column) {
    ...
    if (cells > 35000000) {
      throw new Error(workerT("core.pinvariant.webgpuMatrixTooLarge"));
    }
...
async function resolveDotEngine(accelerationMode) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : workerT("worker.pinvariant.webgpuInitUnknown");
    return {
      requested,
      used: "cpu",
      warning: workerT("core.pinvariant.webgpuFallback", { reason: message }),
      engine: createCpuDotEngine()
    };
  }
}

async function computePinvariantsMartinezSilva(nodes, arcs, mode, accelerationMode, jobId) {
  ...
  if (engineInfo.warning) {
    postMessage({
      ...
      jobId,
      stage: 0,
      total: Math.max(transitionCount, 1),
      message: engineInfo.warning
    });
  }
  ...
  for (let stage = 0; stage < columns.length; stage += 1) {
    ...
    const progressSuffix = selectedMode === "cover-stop"
      ? workerT("worker.pinvariant.completeSuffix", {
          count: completeAtStage.length,
          coverage: coverageAtStage.coveredAllPlaces ? workerT("worker.pinvariant.coverageYes") : ""
        })
      : "";
    ...
    postMessage({
      ...
      message: workerT("worker.pinvariant.stageProgress", {
        stage: processedStages,
        total: columns.length,
...
async function handleCompute(jobId, payload) {
  try {
    const result = pinvariantsCore && typeof pinvariantsCore.computePinvariantsMartinezSilva === "function"
      ? await pinvariantsCore.computePinvariantsMartinezSilva(
        payload.nodes || [],
        payload.arcs || [],
        payload.mode || "cover-stop",
        payload.acceleration || "cpu",
        {
          onProgress(progress) {
            const safe = progress || {};
            postMessage({
              type: "progress",
              jobId,
              stage: Number(safe.stage || 0),
              total: Math.max(1, Number(safe.total || 1)),
              message: safe.message ? String(safe.message) : workerT("worker.pinvariant.running")
            });
          }
        }
      )
      : await computePinvariantsMartinezSilva(
        payload.nodes || [],
        payload.arcs || [],
        payload.mode || "cover-stop",
        payload.acceleration || "cpu",
    ...
    const message = error instanceof Error ? error.message : workerT("worker.pinvariant.unknown");
    postMessage({
      type: "error",
      jobId,
      message
