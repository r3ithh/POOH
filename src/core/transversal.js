(function(root, factory) {
  if (typeof module === "object" && module.exports) {
    ...
    root.PoohTransversalCore = factory(root.PoohI18n);
  }
...
function summarizeAndSelect(hg, strategy, isXtInput, results) {
  ...
  if (bestExact) {
    ...
    if (isXtInput) {
      reason = tr("core.transversal.recommendExactXt");
    } else {
      reason = tr("core.transversal.recommendExactNonXt");
    }
  } else if (bestRegular) {
    recommended = {
      ...bestRegular,
    ...
    reason = tr("core.transversal.recommendRegular");
  } else {
    recommended = {
      method: "none",
      found: false,
    ...
    reason = tr("core.transversal.noCover");
  }
...
function computeTransversals(payload, options) {
  const started = nowMs();
  const safePayload = payload || {};
  const strategy = normalizeStrategy(safePayload.strategy);
  const isXtInput = safePayload.xtrec && typeof safePayload.xtrec.isXt === "boolean"
    ? Boolean(safePayload.xtrec.isXt)
    : null;
  const onProgress = options && typeof options.onProgress === "function" ? options.onProgress : null;

  const hg = buildHypergraph(safePayload);
  if (hg.colCount === 0 && hg.rowCount > 0) {
    throw new Error(tr("core.transversal.noVertices"));
  }
  for (let rowIndex = 0; rowIndex < hg.edges.length; rowIndex += 1) {
    if ((hg.edges[rowIndex] || []).length === 0) {
      throw new Error(tr("core.transversal.emptyEdge", { edge: hg.rowLabels[rowIndex] }));
    }
  }
  ...
  if (runExactXtr) {
    emitProgress(onProgress, "xtr", tr("core.transversal.startXtr"));
    ...
    emitProgress(
      ...
      results.xtr.found && results.xtr.exact
        ? tr("core.transversal.exactFound", { method: "XTR", size: results.xtr.size })
        : tr("core.transversal.exactNotFound", { method: "XTR" })
    );
  }

  if (runExactDlx) {
    emitProgress(onProgress, "dlx", tr("core.transversal.startDlx"));
    ...
    emitProgress(
      ...
      results.dlx.found && results.dlx.exact
        ? tr("core.transversal.exactFound", { method: "DLX", size: results.dlx.size })
        : tr("core.transversal.exactNotFound", { method: "DLX" })
    );
  }

  if (runExactBacktracking) {
    emitProgress(onProgress, "backtracking", tr("core.transversal.startExactBacktracking"));
    ...
    emitProgress(
      ...
      results.backtrackingExact.found && results.backtrackingExact.exact
        ? tr("core.transversal.exactFound", { method: "Backtracking exact", size: results.backtrackingExact.size })
        : tr("core.transversal.exactNotFound", { method: "Backtracking exact" })
    );
  }
  ...
  if (runRegularBacktracking) {
    emitProgress(onProgress, "backtracking", tr("core.transversal.startRegularBacktracking"));
    ...
    emitProgress(
      ...
      results.backtrackingRegular.found && results.backtrackingRegular.coversAll
        ? tr("core.transversal.coverFound", { method: "Backtracking regular", size: results.backtrackingRegular.size })
        : tr("core.transversal.coverNotFound", { method: "Backtracking regular" })
    );
  }

  if (runRegularGreedy) {
    emitProgress(onProgress, "greedy", tr("core.transversal.startGreedy"));
    ...
    emitProgress(
      ...
      results.greedy.found && results.greedy.coversAll
        ? tr("core.transversal.coverFound", { method: "Greedy", size: results.greedy.size })
        : tr("core.transversal.coverNotFound", { method: "Greedy" })
    );
  }
...
return {
  ...
  runXtrGreedyExact,
  runDlxLikeExact,
  runBacktrackingExact,
  runBacktrackingRegular,
  runGreedyRegular,
  wrapMethodResult,
  summarizeAndSelect,
  computeTransversals
